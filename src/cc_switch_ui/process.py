...
class ClaudeProcess:
    ...
    def _on_exit(self):
        ...
        self.start(
            ...
            args=last.get("args"), rows=last.get("rows", 24),
            cols=last.get("cols", 80), cwd=last.get("cwd"),
        )
    ...
    # ---- 生命周期 ----
    def start(self, env_extra, provider_label, args=None, rows=24, cols=80, cwd=None):
        with self.lock:
            if self.is_running():
                return False, "claude 进程已在运行"

        if not shutil.which("claude"):
            return False, "未找到 claude 命令，请先安装 Claude Code"
